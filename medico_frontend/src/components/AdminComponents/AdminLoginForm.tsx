...
const AdminLoginForm: React.FC = () => {
  ...
  const formSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (userDetails) {
        const response = await axiosInstance.post("/admin/login-admin/", userDetails);
  ...
  return (
    <form
      onSubmit={formSubmit}
      className="flex flex-col gap-4 rounded-md text-gray-700 shadow p-4 md:p-12 border-gray-400 border w-full sm:w-[400px]"
    >
      <Toaster visibleToasts={1} position="top-right" richColors />
      <div className="flex ">
        <span className=" font-medium text-xl">Admin</span>
        <span className="pl-2 text-green-700 font-medium text-xl">Login</span>
      </div>
      ...
      <div>
        Doctor login
        <span onClick={()=>navigate('/doctor-login')} className="ml-2 text-green-500 cursor-pointer">
          Click here
        </span>
      </div>
    </form>
